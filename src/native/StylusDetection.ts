...
export function useStylusDetection() {
   ...
   */
  const onTouchDetected = useCallback((touchType: 'stylus' | 'direct' | 'unknown') => {
    const inputType: StylusInputType = 
      touchType === 'stylus' ? 'pencil' : 
      touchType === 'direct' ? 'finger' : 'unknown';
    
    setState(prev => {
      const isPencilDetected = prev.isPencilDetected || inputType === 'pencil';
      return {
        ...prev,
        currentInputType: inputType,
        isPencilDetected,
        // Enable handwriting only if pencil is detected and supported
        isHandwritingEnabled: prev.isSupported && isPencilDetected && inputType === 'pencil',
      };
    });
  }, []);
  ...
  const setHandwritingEnabled = useCallback((enabled: boolean) => {
    setState(prev => ({
      ...prev,
      isHandwritingEnabled: prev.isSupported && prev.isPencilDetected && enabled,
    }));
  }, []);
