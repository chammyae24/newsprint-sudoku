...
export async function recognizeDigitJS(
  ...
  if (__DEV__) {
    ...
    const preview = imageMatrix
      .slice(0, 10)
      .map((row) => row.map((v) => (v > 0.5 ? '█' : ' ')).join(''))
      .join('\n');
    console.log(preview);
  }
  ...
  const sudokuCandidates = topPredictions.filter(
    (p) => p.digit >= 1 && p.digit <= 9
  );
  ...
  if (predictedDigit === 9) {
    ...
    if (looksLike4 && strokeFeatures.hasHorizontalCrossbar) {
      ...
      if (fourCandidate) {
        ...
        if (__DEV__)
          console.log(
            'Post-processing: Changed 9 → 4 (image pattern + crossbar)'
          );
      }
    }
  }
  ...
  if (predictedDigit === 4 && confidenceGap < 0.3) {
    ...
    if (
      ...
      if (__DEV__)
        console.log('Post-processing: Changed 4 → 9 (loop + close second)');
    }
  }
  ...
  if (predictedDigit === 3 && confidenceGap < 0.3) {
    ...
    if (
      ...
      if (__DEV__)
        console.log(
          'Post-processing: Changed 3 → 5 (top horizontal + close second)'
        );
    }
  }
  ...
  if (predictedDigit === 5 && confidenceGap < 0.3) {
    ...
    if (
      ...
      if (__DEV__)
        console.log(
          'Post-processing: Changed 5 → 3 (left open + close second)'
        );
    }
  }
  ...
  if (strokeFeatures.aspectRatio < 0.35 && strokeFeatures.isMainlyVertical) {
    ...
    if (
      oneCandidate &&
      predictedDigit !== 1 &&
      oneCandidate.confidence > 0.05
    ) {
      predictedDigit = 1;
      confidence = Math.max(oneCandidate.confidence, 0.65);
      if (__DEV__)
        console.log('Post-processing: Changed → 1 (narrow vertical shape)');
    }
  }

  // Update candidates with any corrections
  const finalCandidates = sudokuCandidates
    .map((c) => ({
      ...c,
...
function analyze4vs9Pattern(imageMatrix: number[][]): boolean {
  ...
  const checkRowStart = minRow + Math.floor(height * 0.6);
  ...
  const bottomCenterDensity =
    bottomCenterTotal > 0 ? bottomCenterInk / bottomCenterTotal : 0;

  if (__DEV__) {
    console.log(
      `4vs9 pattern: bottomCenterDensity=${bottomCenterDensity.toFixed(3)}, looksLike4=${bottomCenterDensity < 0.25}`
    );
  }
...
function analyzeStrokes(
  ...
  for (const path of paths) {
    for (let i = 1; i < path.points.length; i++) {
      ...
      if (
        (p1.y >= middleYStart && p1.y <= middleYEnd) ||
  ...
  const hasTopHorizontal =
    totalTopPoints > 0 && topHorizontalCount > totalTopPoints * 0.2;
  ...
  const middleRegionEnd = minY + height * 0.7;

  // Check multiple Y slices for horizontal span
  for (
    let checkY = middleRegionStart;
    checkY <= middleRegionEnd;
    checkY += yTolerance
  ) {
    const pointsAtY = allPoints.filter(
      (p) => p.y >= checkY - yTolerance && p.y <= checkY + yTolerance
    );
    ...
    if (pointsAtY.length >= 3) {
      ...
      if (horizontalSpan >= width * 0.4) {
        ...
        if (__DEV__)
          console.log(
            `Crossbar detected at Y=${checkY.toFixed(0)}, span=${horizontalSpan.toFixed(0)}, width=${width.toFixed(0)}`
          );
        break;
      }
    }
  ...
  if (
    ...
    totalMiddlePoints > 0 &&
    middleHorizontalCount > totalMiddlePoints * 0.15
  ) {
    hasHorizontalCrossbar = true;
  }
  ...
  const topCurvature =
    smoothCurveCount / Math.max(1, sharpAngleCount + smoothCurveCount);
